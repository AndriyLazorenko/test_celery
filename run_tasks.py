...
def run_mock_trading():
    ...
    flatten_refresh_gp = group((flatten_prod_unchained.s() | refresh_syms_lots_unchained.s()), (flatten_paper_unchained.s() | refresh_syms_lots_unchained.s()))
    make_step_refresh_model_gp = group((make_step_paper.s() | refresh_model.s()), (make_step_prod.s() | refresh_model.s()))
    result = (wait_for_candle.s() | sleeping.s() | flatten_refresh_gp | reducer.s() | make_step_refresh_model_gp | reducer.s()).apply_async()
    print(result.get())
    print(f'Task finished in {time.time()-t1} seconds')
